...
$dialog show @s {type:multi_action,title:{text:'     Log in',bold:true},body:{type:plain_message,contents:[{text:'Enter the password you created earlier to log into the server. Do NOT enter real passwords from your Minecraft account or other accounts!'},{text: '\n\n $(psw) ',underlined:true}]},can_close_with_escape:false,pause:false,after_action:none,columns:11,actions:[\
...
{label:'',width:20},\
{label:'←—┚',width:40,action:{type:run_command,command:'trigger xhl.unikey set 40'}},\
{label:'',width:20},\
